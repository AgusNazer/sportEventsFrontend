...
export default function EventCard({ event }: Props) {
  ...
  return (
    <Link href={`/events/${event.slug}`} className="block group">
      <article className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
        ...
        <div className="p-5">
          ...
          {/* Nombre */}
          <h3 className="font-black text-gray-900 text-base leading-snug tracking-tight group-hover:text-brand-700 transition-colors mb-4 uppercase">
            {event.nombre}
          </h3>

          {/* Fecha + lugar en dos columnas */}
          ...
          {event.precio && (
            <div className="mt-4 pt-3 border-t border-gray-100 flex justify-end">
              <span className="text-sm font-bold text-gray-900">{event.precio}</span>
            </div>
          )}
        </div>
      </article>
